...
- Bumped the `Wolfgang.Etl.Abstractions` family to **0.23.2**
  ...
  projects). No shipped-API change — 0.23.1 was a security + test-code-hardening
  release, and 0.23.2 is a maintenance wave (CodeQL SHA re-pin fixing the OSSF
  Scorecard badge, mutation-test InspectCode cleanup, `System.Text.Json` 8.0.5
  in the upstream Coyote test project, first-party 10.0.x deps to 10.0.11,
  analyzer refresh, per-module `>=99%` unit-test coverage floor).
