...
public sealed class FixedWidthExtractorCheckpointTests
{
    ...
    public async Task Tracking_with_a_bomless_encoding_reports_offsets_from_zero()
    {
        ...
        using var extractor = new FixedWidthExtractor<Rec>(new MemoryStream(Ascii(ThreeRecords)), new FixedWidthExtractorStreamOptions<Rec> { Encoding = Encoding.ASCII }) { TrackByteOffset = true };
