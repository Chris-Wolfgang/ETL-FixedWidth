...
namespace Wolfgang.Etl.FixedWidth.Tests.Unit
{
    ...
    public class FixedWidthExtractorTests
    {
        ...
        private static FixedWidthExtractor<PersonRecord, Report> CreateExtractor(string content)
            => new FixedWidthExtractor<PersonRecord, Report>(new StringReader(content));
        ...
        {
            var extractor = CreateExtractor( "John      Smith     042\n" + "END\n" + "Jane      Doe       030");
            extractor.LineFilter = line => line == "END"
                ? LineAction.Stop
                : LineAction.Process;
        ...
        public async Task ExtractAsync_LineFilter_is_not_invoked_for_blank_lines()
        {
            ...
            {
                if (string.IsNullOrEmpty(line)) filterInvokedForBlank = true;
        ...
        [Fact]
        public async Task ExtractAsync_default_ValueParser_is_FixedWidthConverter_DefaultParser()
        {
            var extractor = CreateExtractor(string.Empty);
        ...
        private static FixedWidthExtractor<EmployeeRecord, Report> CreateEmployeeExtractor(string content)
            => new FixedWidthExtractor<EmployeeRecord, Report>(new StringReader(content));


        private static async Task<List<EmployeeRecord>> ExtractAllEmployees( FixedWidthExtractor<EmployeeRecord, Report> extractor)
        {
            var results = new List<EmployeeRecord>();
            await foreach (var r in extractor.ExtractAsync())
                results.Add(r);
            return results;
        }
        ...
        public async Task ExtractAsync_when_FixedWidthSkip_attributes_are_present_parses_the_correct_fields()
        {
            ...
            var line = "John      19900101202301012345 Smith     ";
            var extractor = CreateEmployeeExtractor(line);

            var results = await ExtractAllEmployees(extractor);
        ...
        public async Task ExtractAsync_when_FixedWidthSkip_attributes_are_present_expected_line_width_includes_skip_column_widths()
        {
            ...
            var line = "John      2345 Smith     "; // only 25 chars, need 41
            var extractor = CreateEmployeeExtractor(line);

            await Assert.ThrowsAsync<LineTooShortException>(async () => await ExtractAllEmployees(extractor));
        }
        ...
        {
            var extractor = new FixedWidthExtractor<PersonRecord, System.Exception>(new StringReader(string.Empty));
