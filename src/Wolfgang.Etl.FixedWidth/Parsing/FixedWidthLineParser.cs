...
namespace Wolfgang.Etl.FixedWidth.Parsing
{
    ...
    internal static class FixedWidthLineParser
    {
        ...
        )
        {
            ...
            for (var i = 0; i < descriptors.Count; i++)
            {
                ...
                if (text.Length > attr.Length)
                {
                    ...
                    (
                        $"The value converter returned a string of length {text.Length} for property " + $"'{prop.Name}' which exceeds the defined field width of {attr.Length}. " + $"Ensure your ValueConverter honors the FieldLength in FieldContext.",
                        prop.Name,
                        attr.Length,
                        text.Length
                    );
            ...
            where T : new()
        {
            ...
            var fullExpectedWidth = fieldMap.ExpectedLineWidth
                                  + (delimiterWidth * (fieldMap.TotalColumnCount - 1));

            if (line.Length < fullExpectedWidth)
            {
                throw new LineTooShortException
                (
                    $"Line {lineNumber} is too short. Expected {fullExpectedWidth} characters" + (delimiterWidth > 0 ? $" ({fieldMap.ExpectedLineWidth} field/skip width" + $" + {delimiterWidth * (fieldMap.TotalColumnCount - 1)} delimiter width)" : string.Empty) + $" but found {line.Length}.",
                    lineNumber,
                    line,
                    fullExpectedWidth,
                    line.Length
            ...
            for (var i = 0; i < descriptors.Count; i++)
            {
                ...
                var start = descriptors[i].Start
                          + (delimiterWidth * descriptors[i].AbsoluteColumnIndex);
