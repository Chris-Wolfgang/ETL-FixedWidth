    ...
    public static class FixedWidthConverter
    {
        ...
        )
        {
            ...
            if (value is DateTime || value is DateTimeOffset || value is TimeSpan)
            {
                ...
                {
                    throw new InvalidOperationException( $"Property '{context.PropertyName}' is of type '{value.GetType().Name}' but no " + $"Format was specified on the [FixedWidthField] attribute. Fixed-width date and " + $"time fields require an explicit format string (e.g. \"yyyyMMdd\", \"HHmmss\").");
                }
        ...
        )
        {
            ...
            if (targetType == typeof(DateTime) || targetType == typeof(DateTimeOffset) || targetType == typeof(TimeSpan))
            {
                ...
                {
                    throw new InvalidOperationException( $"Cannot parse a value of type '{targetType.Name}' without a format string. " + $"Specify a Format on the [FixedWidthField] attribute (e.g. \"yyyyMMdd\", \"HHmmss\").");
                }
