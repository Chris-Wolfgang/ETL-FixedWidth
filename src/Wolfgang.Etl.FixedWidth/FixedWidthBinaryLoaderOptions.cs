...
/// <summary>
/// Options for the <see cref="System.IO.Stream"/>-based <see cref="FixedWidthBinaryLoader{TRecord}"/> constructors.
/// </summary>
/// <remarks>
...
/// </para>
/// </remarks>
public sealed record FixedWidthBinaryLoaderOptions
{
    /// <summary>
    /// Gets the <see cref="System.Text.Encoding"/> used to encode text fields into each binary record.
    ...
    /// <summary>
    /// Gets the maximum number of items the stage loads. Defaults to <see cref="int.MaxValue"/>.
    /// </summary>
    /// <exception cref="ArgumentOutOfRangeException">The value is less than <c>1</c>.</exception>
    ...
    /// <summary>
    /// Gets the number of items the stage skips before the first is loaded. Defaults to <c>0</c>.
    /// </summary>
    ...
    private static readonly Func<ItemErrorContext, ItemErrorAction> DefaultErrorPolicy = new LoaderOptions().ErrorPolicy;
    ...
    /// <see cref="LoaderBase{TDestination, TProgress}"/>. The record does not <i>derive</i> from
    /// <see cref="LoaderOptions"/> on purpose: on net462 / net481 / netstandard2.0 a derived record's
    ...
    /// </summary>
    internal LoaderOptions ToLoaderOptions() =>
        new()
        {
